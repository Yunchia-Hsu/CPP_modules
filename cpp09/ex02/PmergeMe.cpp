...
/*   Updated: 2025/02/13 15:24:59 by yhsu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//接著依據 Jacobsthal 數列決定的順序
    ...
    for (size_t i = 0; i < b.size(); i++)
    {
		...
		if (guessIndex > tot)
        	guessIndex = tot;
		auto startIt = s.begin() + guessIndex;
		auto targetIt = std::upper_bound(startIt, s.end(), b[i]);
        //int targetIndex = std::upper_bound(startIt, s.end(), b[i]) - s.begin();
        

		//int targetIndex = targetIt - s.begin();
		  // 在正確位置插入元素
   		s.insert(targetIt, b[i]);
